...
export async function POST(request: Request) {
  ...
  const { entries } = (await request.json()) as {
    entries?: { login: string; seconds: number; active?: boolean; hidden?: boolean }[];
  };
  ...
  for (const entry of entries) {
    ...
    // Часы идут всем, а «был в сети» не отмечаем невидимой администрации:
    // счётчик онлайна на сайте считается именно по нему, и скрытый админ не
    // должен в нём светиться.
    const result = await db.user.updateMany({
      where: { login: entry.login },
      data: {
        playtimeSec: { increment: seconds },
        ...(entry.hidden ? {} : { lastSeenAt: new Date() }),
      },
    });
